...
from .convert import load_converted_model, load_converted_model_without_llama
...
class UpstreamExpertWithoutLlama(UpstreamBase):
    def __init__(self, ckpt, **kwargs):
        super().__init__(**kwargs)
        model, task_cfg = load_converted_model_without_llama(ckpt)
        #model, task_cfg = load_and_convert_fairseq_ckpt(ckpt)
        self.model = model
        self.task_cfg = task_cfg
    ...
    def forward(self, wavs):
        ...
        features, feat_padding_mask = self.model.extract_features_remove_llama(
            padded_wav,
            padding_mask=wav_padding_mask,
            mask=None,
        )
